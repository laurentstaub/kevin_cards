// FlashPharma Admin JavaScript - Refactored with FormModule
class AdminApp {
  constructor() {
    this.apiBase = '/api'; // Keep for backward compatibility
    this.currentPage = 1;
  ...
  setupEventListeners() {
    document.addEventListener('click', (e) => {
      ...
      switch (action) {
        ...
        case 'close-question-modal': ModalManager.hide('question-modal'); break;
        case 'cancel-question': ModalManager.hide('question-modal'); break;
        ...
        case 'add-source': FormModule.sources.add(); break;
    ...
    Object.entries(inputHandlers).forEach(([id, handler]) => {
      ...
      if (element) {
        ...
        const finalHandler = isTagInput ? handler : UIHelpers.debounce(handler, debounceTime);
  ...
  async loadInitialData() {
    try {
      const tagsResponse = await ApiClient.tags.getAll();
      ...
      UIHelpers.toast('Erreur lors du chargement des données', 'error');
    }
  }
  ...
  // Modal management - now using ModalManager
  ...
  // Form management - now using FormModule
  resetQuestionForm() {
  ...
  // Source management - now using FormModule
  addSource() {
    return FormModule.sources.add();
  }
  ...
  async saveQuestion() {
    ...
    // Add selected tags
    questionData.tagIds = this.selectedTags.filter(tag => tag.id).map(tag => tag.id);
  ...
  // Source management methods - now using FormModule
  collectSources() {
    return FormModule.sources.collect();
  }
  ...
  // API methods - now delegating to ApiClient
  ...
  // Utility methods - now delegating to UIHelpers
  showLoading(show) {
  ...
  async loadTagForEdit(tagId) {
    try {
      const tag = await ApiClient.tags.getById(tagId);
      ...
      UIHelpers.toast('Erreur lors du chargement du tag', 'error');
    }
  }

  // Source management - delegated to FormModule
  loadSources(sources) {
    FormModule.sources.load(sources);
  }

  createSourceItem() {
    return FormModule.sources.create();
  }
